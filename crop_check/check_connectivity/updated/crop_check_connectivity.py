...
def get_neuroglancer_url(row, input_zarr_path):
    ...
    with viewer.txn() as s:
        ...
        crop_path = f"{input_zarr_path}/{collection}/groundtruth.zarr/{crop}"

        for organelle_path in glob.glob(f"{crop_path}/*"):
            ...
            try:
                segments = [
                    ...
                    for mesh_id in os.listdir(
                        f"/nrs/cellmap/ackermand/new_meshes/meshes/crop_check_updated/{collection}/{crop}/{organelle_name}/meshes"
                    )
                    if ":0" in mesh_id
                ]
            except:
                ...
                continue
            image_source = f'zarr://https://cellmap-vm1.int.janelia.org/{crop_path.replace("/nrs/cellmap/","/nrs/")}/{organelle_name}'
            mesh_source = f"precomputed://https://cellmap-vm1.int.janelia.org/nrs/ackermand/new_meshes/meshes/crop_check_updated/{collection}/{crop}/{organelle_name}/meshes"
            ...
            if not have_set_position:
                ds = open_ds(
                    raw_zarr_path,
                    f"recon-1/labels/groundtruth/{crop}/{organelle_name}/s0",
                )
...
def process_rows(rows, connectivity):
        ...
        for _, row in rows.iterrows():
            ...
            try:
                idi = ImageDataInterface(
                    f"/nrs/cellmap/zubovy/crop_splits/combined/{collection}/groundtruth.zarr/crop{crop_number}/{organelle_name}/s0"
                )
...
# get local ip
@click.command()
@click.option("--input_zarr_path", "-z", type=str, prompt="Path to crop zarrs")
@click.option("--input_csv", "-i", type=str, prompt="Path to csv")
@click.option("--output_dir", "-o", type=str, prompt="Path to output directory")
@click.option("--connectivity", "-c", type=int, help="Your age.")
def main(input_zarr_path, input_csv, output_dir, connectivity):
    os.makedirs(output_dir, exist_ok=True)
    original_df = pd.read_csv(input_csv)
    new_df_data = []
    for _, row in original_df.iterrows():
        url = get_neuroglancer_url(row, input_zarr_path)
        crop_number = row["crop_number"]
        collection = row["collection"]
        # crop_path = f"/nrs/cellmap/zubovy/crop_splits/combined/{collection}/groundtruth.zarr/crop{crop_number}"
        crop_path = f"{input_zarr_path}/{collection}/groundtruth.zarr/crop{crop_number}"
        for organelle_path in glob.glob(f"{crop_path}/*"):
            organelle_name = os.path.basename(organelle_path)
    ...
    cluster = LocalCluster(
        n_workers=int(os.cpu_count() / 8), threads_per_worker=1, memory_limit="64GB"
    )
    with Client(cluster) as client:
        ...
        result_ddf = ddf.map_partitions(
            process_rows, connectivity=connectivity, meta=ddf
        )
        output_df = result_ddf.compute()

        output_df.to_csv(
            f"{output_dir}/isolated_pixels_connectivity_{connectivity}.csv", index=False
        )
