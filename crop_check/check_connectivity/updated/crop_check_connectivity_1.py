...
def get_neuroglancer_url(row):
    collection = row['collection']
    zarr_path = f"/nrs/cellmap/data/{collection}/{collection}.zarr" 
...
def process_rows(rows):
    ...
    try:
        for _, row in rows.iterrows():
            ...
            reconnected = skimage.measure.label(data > 0, connectivity=1)
...
if __name__ == "__main__":
    ...
    new_df = pd.DataFrame(new_df_data, columns=["collection","crop_number","organelle_name","url","small_object_ids"])
    ...
    with Client(cluster) as client:
        ...
        output_df.to_csv("isolated_pixels_connectivity_1.csv", index=False)
